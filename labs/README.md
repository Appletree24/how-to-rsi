...
另一个可独立运行的实验是[改进器自应用](improver/README.md)：实际执行时长解析器，对照冻结改进器、配置自应用与穷举，记录嵌套成本和分布变化后的退化。
...
命令成功读取并比较时退出 0，输入不合法时退出 2；候选是否通过由 JSON 中的 `decision` 表示。

## 选择偏差实验

所有候选真实成功率固定为 .5；在 40 个开发观测上选最高分，再独立生成 40 个确认观测评估该候选，重复 300 次。seed=17、candidates=30 时，Python 实验得到开发均值约 .6592、新数据均值 .5030。两者的差异来自选择过程，候选的真实成功率没有变化。
