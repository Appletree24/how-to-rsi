...
function parseSnapshot(value: unknown): NotebookSnapshot | null {
  if (!isRecord(value) || typeof value.salt !== 'string' || !value.salt ||
      typeof value.verify !== 'string' || !value.verify ||
      (value.payload !== undefined && typeof value.payload !== 'string')) return null;
  return { salt: value.salt, verify: value.verify, payload: value.payload };
}
function parseNotes(value: unknown): NotebookNote[] {
  if (!Array.isArray(value)) throw new Error('Invalid notebook contents');
  return value.map((entry: unknown) => {
    if (!isRecord(entry) || typeof entry.id !== 'string') throw new Error('Invalid notebook note');
    const field = (name: string): string => {
      const value = entry[name];
      if (value === undefined || value === null) return '';
      if (typeof value !== 'string') throw new Error('Invalid notebook field: ' + name);
      return value;
    };
    return { id: entry.id, num: field('num'), title: field('title'), diff: field('diff'),
      tags: field('tags'), link: field('link'), idea: field('idea'), code: field('code'), note: field('note') };
  });
}
function $<T extends HTMLElement = HTMLElement>(selector: string, root: ParentNode = document): T | null {
  return root.querySelector<T>(selector);
}
function required<T extends HTMLElement = HTMLElement>(selector: string, root: ParentNode = document): T {
  const node = $<T>(selector, root);
  if (!node) throw new Error('Missing required application element: ' + selector);
  return node;
}
...
function buttonize<T extends HTMLElement>(n: T, role?: string): T {
  n.setAttribute('role', role || 'button');
  n.tabIndex = 0;
  return n;
}
// Enter/Space 触发 click(click() 也会走已注册的 addEventListener)
function keyActivate(n: HTMLElement): void {
...
function icon(name: string, size?: number): string {
  var s = size || 18;
  return '<svg width="' + s + '" height="' + s + '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="' + ((Object.hasOwn(DSH.icons, name) ? DSH.icons[name as keyof typeof DSH.icons] : undefined) || DSH.icons.info) + '"/></svg>';
}
...
/* ---------- 持久化进度 ---------- */
var store: ProgressStore = { done: {}, theme: 'light', best: -1 };
try {
  var raw = localStorage.getItem('dsh-learn-v1');
  if (raw) {
    const parsed: unknown = JSON.parse(raw);
    if (isRecord(parsed)) {
      if (isRecord(parsed.done)) {
        store.done = Object.fromEntries(Object.entries(parsed.done).filter((entry): entry is [string, boolean] => typeof entry[1] === 'boolean'));
      }
      store.theme = parsed.theme === 'dark' ? 'dark' : 'light';
      store.best = typeof parsed.best === 'number' ? parsed.best : -1;
    }
  }
} catch (e) { /* storageDenied: 隐私模式下可能禁用,降级为不持久 */ }
function save() {
...
var currentId = 'home';
function buildNav() {
  var nav = required('#navList');
  nav.innerHTML = '';
  var lastModule: string | null = null;
...
function go(id: string): void { location.hash = '#/' + id; }
function route() {
...
function buildSim() {
  ...
  var swEl = buttonize(required('#simShort'), 'switch');
  ...
  keyActivate(swEl);
  function logLine(html: string): void { var lg = required('#simLog'); lg.innerHTML += '<div>» ' + html + '</div>'; lg.scrollTop = lg.scrollHeight; }
  function reset() {
    timers.forEach(clearTimeout); timers = [];
    required('#simLog').innerHTML = '';
    required('#simResult').textContent = '结果：…';
    for (var i = 1; i <= 4; i++) { var n = required('#simN' + i); n.className = 'sim-node'; required('.rl', n).textContent = '监听器'; }
  }
  function at(ms: number, fn: () => void): void { timers.push(window.setTimeout(fn, reducedMotion ? 0 : ms)); }
  function setN(i: number, cls: string, role?: string): void { var n = required('#simN' + i); n.className = 'sim-node ' + cls; if (role) required('.rl', n).textContent = role; }
  function describe() {
    var m = DSH.dispatchModes.find(function (x) { return x.id === mode; });
    if (!m) throw new Error('Unknown dispatch mode: ' + mode);
    required('#simLog').innerHTML = '<div>» <span class="hl2">' + m.id + '</span>：' + m.desc + '</div>';
    required('#simResult').textContent = '结果：—';
  }
  describe();
  required('#simRun').addEventListener('click', function () {
    reset();
    ...
    if (mode === 'emit') {
      ...
      at(60, function () { required('#simResult').textContent = '结果：无返回值,分发后立即返回'; logLine('<span class="warn2">分发方已继续执行 —— emit 不等待监听器</span>'); });
      ...
      at(step * 2, function () {
        ...
        if (shortC) {
          ...
          required('#simResult').textContent = '结果：L3 的返回值';
        } else {
          setN(3, 'lit', '直接 next()');
      ...
      if (!shortC) {
        ...
        at(step * 4, function () { setN(4, 'done3'); logLine('值沿 next() 的返回值逐层回传、层层可被替换'); required('#simResult').textContent = '结果：层层包装后的返回值'; });
      }
      ...
      at(500 + 4 * 300, function () { required('#simResult').textContent = '结果：无返回值(全部完成后 resolve)'; });
    } else if (mode === 'serial') {
      [1, 2, 3, 4].forEach(function (i, k) {
        at(step * k, function () { setN(i, 'lit', 'await 中'); logLine('await L' + i + ' …'); });
        at(step * k + 460, function () { setN(i, 'done3', '完成'); });
      });
      at(step * 4, function () { required('#simResult').textContent = '结果：有返回值,逐个 await'; });
    } else if (mode === 'bail') {
      ...
      at(step * 2, function () { setN(2, 'done3'); setN(3, 'bailed', '返回了值!'); setN(4, 'skip', '未观察'); logLine('<span class="warn2">L3 返回非空值 —— bail,分发终止</span>'); required('#simResult').textContent = '结果：L3 的返回值'; });
    }
  });
}
...
function buildLoop() {
  ...
  var dots = required('#loopDots'), stepsBox = required('#loopSteps');
  ...
  function render() {
    ...
    required('#loopNarr').innerHTML = '<b>第 ' + (idx + 1) + '/' + DSH.loopSteps.length + ' 帧 · ' + DSH.loopSteps[idx].title + '</b><br>' + DSH.loopSteps[idx].narr;
  }
  function stopPlay() {
    if (timer) { clearInterval(timer); timer = null; required('#loopPlay').innerHTML = '▶ 自动播放'; }
  }
  required('#loopPrev').addEventListener('click', function () { stopPlay(); idx = Math.max(0, idx - 1); render(); });
  required('#loopNext').addEventListener('click', function () { stopPlay(); idx = Math.min(DSH.loopSteps.length - 1, idx + 1); render(); });
  required('#loopReset').addEventListener('click', function () { stopPlay(); idx = 0; render(); });
  required('#loopPlay').addEventListener('click', function () {
    if (timer) { stopPlay(); return; }
    if (idx >= DSH.loopSteps.length - 1) idx = 0;
    required('#loopPlay').innerHTML = '⏸ 暂停';
    render();
...
function buildExtPicker() {
  const host = $('#extPicker');
  if (!host) return;
  var sel = el('select', 'sel-input');
  DSH.extGoals.forEach(function (g, i) {
    var o = el('option', '', ''); o.value = String(i); o.textContent = g.g; sel.appendChild(o);
  });
  var out = el('div', 'picker-out');
  function show() {
    var g = DSH.extGoals[Number(sel.value) | 0];
...
function buildEvoSim() {
  ...
  var BITS = 8;
  function score(bits: number[]): number {
  ...
  var evoTimer: number | null = null;
  var cells = required('#evoGrid');
  var archive: EvolutionAgent[] = [];
  var cellEls: HTMLDivElement[] = [];
  for (var c = 0; c < 16; c++) {
  ...
  function logLine(html: string): void { var lg = required('#evoLog'); lg.innerHTML += '<div>» ' + html + '</div>'; lg.scrollTop = lg.scrollHeight; }
  function mutate(bits: number[]): number[] {
    var nb = bits.slice();
  ...
  function run(greedy: boolean): void {
    ...
    evoTimer = window.setInterval(function () {
      ...
      render();
      if (gen >= 60) {
        if (evoTimer !== null) clearInterval(evoTimer); evoTimer = null;
        var b = best();
        var verdict = greedy
        ...
        required('#evoResult').textContent = '结果：' + verdict;
        logLine('<span class="hl2">最优 ' + b.score + ' 分</span> — ' + verdict);
      }
    }, reducedMotion ? 5 : 55);
  }
  required('#evoRun').addEventListener('click', function () { run(false); });
  required('#evoGreedy').addEventListener('click', function () { run(true); });
  required('#evoReset').addEventListener('click', function () {
    if (evoTimer) { clearInterval(evoTimer); evoTimer = null; }
    archive = []; render();
    required('#evoLog').innerHTML = '每个格子是一个 agent：颜色越深分数越高(金框 = 全局最优)。开放档案可从任意祖先分支；贪心只能改当前最强。';
    required('#evoResult').textContent = '选择一个策略开始';
  });
  render();
}
...
function renderExp() {
  ...
  DSH.experiments.forEach(function (x) {
    ...
    if (on) done++;
    var cb = $<HTMLInputElement>('#expcb-' + x.id);
    if (cb) cb.checked = on;
    var card = $('#exp-' + x.id);
...
function buildEvalGate() {
  ...
  function logLine(html: string): void { var lg = required('#egLog'); lg.innerHTML += '<div>» ' + html + '</div>'; lg.scrollTop = lg.scrollHeight; }
  function roll() {
    ver++;
    var before = passing.slice();
    var desc: string, delta = 0;
    var failIdx: number[] = [], passIdx: number[] = [];
    ...
    renderTasks();
    required('#egResult').textContent = '基线 ' + baseline + '/7';
  }
  required('#egRun').addEventListener('click', roll);
  required('#egAuto').addEventListener('click', function () { var n = 0; var t = setInterval(function () { roll(); if (++n >= 5) clearInterval(t); }, reducedMotion ? 10 : 350); });
  required('#egCheat').addEventListener('click', function () { cheat = !cheat; this.classList.toggle('on2', cheat); this.setAttribute('aria-checked', cheat ? 'true' : 'false'); });
  required('#egSent').addEventListener('click', function () { sentinel = !sentinel; this.classList.toggle('on2', sentinel); this.setAttribute('aria-checked', sentinel ? 'true' : 'false'); });
  var swCheat = buttonize(required('#egCheat'), 'switch'); keyActivate(swCheat);
  var swSent = buttonize(required('#egSent'), 'switch'); keyActivate(swSent);
  swCheat.classList.add('on2'); swCheat.setAttribute('aria-checked', 'true');
  swSent.setAttribute('aria-checked', 'false');
  required('#egReset').addEventListener('click', function () {
    passing = [true, true, true, true, true, false, false]; baseline = 5; ver = 0;
    required('#egLog').innerHTML = '当前版本 v0:能对 5/7 任务(基线 5 分)。注入自改后跑评测:报告分 ≥ 基线则保留,否则回滚。';
    required('#egResult').textContent = '';
    renderTasks();
  });
  renderTasks();
}
...
function buildPkgExplorer() {
  const host = $('#pkgExplorer');
  if (!host) return;
  var bucket = 'all', query = '';
  ...
  required<HTMLInputElement>('#pkgSearch').addEventListener('input', function () { query = this.value.trim().toLowerCase(); render(); });
  function render() {
    var box = required('#pkgGroups');
    box.innerHTML = '';
    ...
    required('#pkgCount').textContent = gCount + ' 组 · ' + pCount + ' 包';
...
function buildToolExplorer() {
  ...
  required<HTMLInputElement>('#toolSearch').addEventListener('input', function () { query = this.value.trim().toLowerCase(); render(); });
  function render() {
    var grid = required('#toolGrid');
    grid.innerHTML = '';
    var n = 0;
    ...
    required('#toolCount').textContent = n + ' 项';
...
function buildCommands() {
  ...
  groups.forEach(function (g) {
    ...
    DSH.commands.filter(function (c) { return c.g === g; }).forEach(function (c) {
      ...
      required('td:last-child', tr).appendChild(btn);
      tb.appendChild(tr);
    });
    tbl.appendChild(tb);
...
function buildQuiz() {
  ...
  function answer(k: number, opts: HTMLElement): void {
    var q = DSH.quiz[i];
    var right = k === q.a;
    results.push(right);
    if (right) score++;
    $$<HTMLButtonElement>('.quiz-opt', opts).forEach(function (b, idx2) {
      b.disabled = true;
...
var LC: NotebookState = { key: null, notes: [], sel: null, salt: null, verify: null };
...
function b64e(buf: ArrayBuffer | Uint8Array): string {
  ...
  for (var i = 0; i < u8.length; i += CH) s += String.fromCharCode(...u8.subarray(i, i + CH));
  return btoa(s);
}
function b64d(s: string): Uint8Array<ArrayBuffer> { return Uint8Array.from(atob(s), function (c) { return c.charCodeAt(0); }); }
function lcStore(): NotebookSnapshot | null {
  try { return parseSnapshot(JSON.parse(localStorage.getItem(LC_KEY) || 'null')); } catch (e) { return null; }
}
...
async function lcPersist(): Promise<boolean> {
  try {
    const { key, salt, verify } = LC;
    if (!key || !salt || !verify) return false;
    var payload = await lcEncrypt(key, JSON.stringify(LC.notes));
    const snapshot: NotebookSnapshot = { salt: b64e(salt), verify, payload };
    localStorage.setItem(LC_KEY, JSON.stringify(snapshot));
    return true;
  } catch (e) { return false; }
}
function lcMsg(t: string, isErr = false): void { var m = $('#lcGateMsg'); if (m) { m.textContent = t; m.style.color = isErr ? 'var(--rose)' : 'var(--muted)'; } }
async function lcDerive(pass: string, salt: Uint8Array<ArrayBuffer>): Promise<CryptoKey> {
  var km = await crypto.subtle.importKey('raw', te.encode(pass), 'PBKDF2', false, ['deriveKey']);
  return crypto.subtle.deriveKey(
    { name: 'PBKDF2', salt: salt, iterations: 250000, hash: 'SHA-256' },
    km, { name: 'AES-GCM', length: 256 }, false, ['encrypt', 'decrypt']);
}
async function lcEncrypt(key: CryptoKey, plain: string): Promise<string> {
  var iv = crypto.getRandomValues(new Uint8Array(12));
  var ct = await crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, key, te.encode(plain));
  return b64e(iv) + '.' + b64e(ct);
}
async function lcDecrypt(key: CryptoKey, payload: string): Promise<string> {
  var p = payload.split('.');
...
async function lcUnlockWith(pass: string, st: NotebookSnapshot | null): Promise<CryptoKey | null> {
  if (!st) return null;
  try {
    var key = await lcDerive(pass, b64d(st.salt));
    if ((await lcDecrypt(key, st.verify)) !== 'ok') return null;
    const notes = st.payload ? parseNotes(JSON.parse(await lcDecrypt(key, st.payload))) : [];
    LC.key = key;
...
function lcShow(main: boolean): void {
  required('#lcGate').hidden = main;
  required('#lcMain').hidden = !main;
  if (main) renderLcList();
}
function lcGateMode(setup: boolean): void {
...
function renderLcList() {
  var q = (required<HTMLInputElement>('#lcSearch').value || '').toLowerCase();
  var box = required('#lcList');
  box.innerHTML = '';
  var list = LC.notes.filter(function (n) {
    if (!q) return true;
    return (n.num + ' ' + n.title + ' ' + n.tags + ' ' + n.idea).toLowerCase().indexOf(q) >= 0;
  });
  required('#lcCount').textContent = '共 ' + LC.notes.length + ' 题' + (q ? ' · 命中 ' + list.length : '');
...
function fillLcEditor(n: NotebookNote): void {
  required('#lcEmpty').hidden = true;
  required('#lcEditor').hidden = false;
  required<HTMLInputElement>('#lcNum').value = n.num || ''; required<HTMLInputElement>('#lcTitle').value = n.title || '';
  required<HTMLSelectElement>('#lcDiff').value = n.diff || 'Medium'; required<HTMLInputElement>('#lcTags').value = n.tags || '';
  required<HTMLInputElement>('#lcLink').value = n.link || ''; required<HTMLTextAreaElement>('#lcIdea').value = n.idea || '';
  required<HTMLTextAreaElement>('#lcCode').value = n.code || ''; required<HTMLTextAreaElement>('#lcNote').value = n.note || '';
  required('#lcSavedHint').textContent = '';
}
function buildLC() {
  if (!$('#ch-lc')) return;
  // WebCrypto 只在 HTTPS 或 localhost 等 secure context 可用 → 明确提示而非静默失败
  if (!window.crypto || !window.crypto.subtle) {
    required('#lcGateInfo').innerHTML = '<span class="t">当前环境不支持加密</span>这个页面需要 WebCrypto(secure context)。请用 <code>npm run dev</code> 起本地服务后访问终端显示的 localhost 地址,或换新版 Chrome/Firefox/Safari 打开。';
    required<HTMLInputElement>('#lcPass').disabled = required<HTMLInputElement>('#lcPass2').disabled = required<HTMLButtonElement>('#lcUnlock').disabled = true;
    return;
  }
  var st = lcStore();
  ...
  required<HTMLButtonElement>('#lcUnlock').addEventListener('click', async function () {
    var p1 = required<HTMLInputElement>('#lcPass').value;
    if (setup) {
      if (p1.length < 10) { lcMsg('口令至少 10 位——它是加密密钥的源头,太短能被离线穷举', true); return; }
      if (p1 !== required<HTMLInputElement>('#lcPass2').value) { lcMsg('两遍口令不一致', true); return; }
  ...
  required<HTMLInputElement>('#lcPass').addEventListener('keydown', function (e) { if (e.key === 'Enter') required<HTMLButtonElement>('#lcUnlock').click(); });
  required<HTMLInputElement>('#lcPass2').addEventListener('keydown', function (e) { if (e.key === 'Enter') required<HTMLButtonElement>('#lcUnlock').click(); });

  required('#lcNew').addEventListener('click', function () {
    var n = { id: 'n' + Date.now(), num: '', title: '', diff: 'Medium', tags: '', link: '', idea: '', code: '', note: '' };
    LC.notes.unshift(n); LC.sel = n.id; fillLcEditor(n); renderLcList(); required<HTMLInputElement>('#lcTitle').focus();
  });
  required('#lcSave').addEventListener('click', async function () {
    var n = LC.notes.find(function (x) { return x.id === LC.sel; });
    if (!n) return;
    n.num = required<HTMLInputElement>('#lcNum').value.trim(); n.title = required<HTMLInputElement>('#lcTitle').value.trim();
    n.diff = required<HTMLSelectElement>('#lcDiff').value; n.tags = required<HTMLInputElement>('#lcTags').value.trim();
    n.link = required<HTMLInputElement>('#lcLink').value.trim(); n.idea = required<HTMLTextAreaElement>('#lcIdea').value;
    n.code = required<HTMLTextAreaElement>('#lcCode').value; n.note = required<HTMLTextAreaElement>('#lcNote').value;
    if (!(await lcPersist())) { required('#lcSavedHint').textContent = '写入失败(存储配额满?),数据未落盘'; return; }
    renderLcList();
    required('#lcSavedHint').textContent = '已保存(密文落盘) ' + new Date().toLocaleTimeString();
  });
  required('#lcDel').addEventListener('click', async function () {
    ...
    if (!(await lcPersist())) {
      ...
      required('#lcSavedHint').textContent = '写入失败,删除未落盘';
      renderLcList(); return;
    }
    LC.sel = null;
    required('#lcEditor').hidden = true; required('#lcEmpty').hidden = false; renderLcList();
  });
  required<HTMLInputElement>('#lcSearch').addEventListener('input', renderLcList);
  required('#lcLock').addEventListener('click', function () {
    LC.key = null; LC.sel = null; LC.notes = []; required<HTMLInputElement>('#lcPass').value = ''; required<HTMLInputElement>('#lcPass2').value = '';
    lcGateMode(false); lcShow(false);
  });
  // 导出:落的是 {salt, verify, payload}——payload 是整包密文,口令不变即可在别处导入解开
  required('#lcExport').addEventListener('click', async function () {
    const { key, salt, verify } = LC;
    if (!key || !salt || !verify) return;
    var payload = await lcEncrypt(key, JSON.stringify(LC.notes));
    const snapshot: NotebookSnapshot = { salt: b64e(salt), verify, payload };
  ...
  required('#lcImport').addEventListener('click', function () { required<HTMLInputElement>('#lcImportFile').click(); });
  required<HTMLInputElement>('#lcImportFile').addEventListener('change', function () {
    var f = this.files?.[0]; if (!f) return;
    var r = new FileReader();
    r.onload = function () {
      try {
        if (typeof r.result !== 'string') throw new Error('Invalid notebook file');
        var d = parseSnapshot(JSON.parse(r.result));
        if (!d || !d.payload) throw new Error('Invalid notebook snapshot');
        // 导入的是密文包:替换本地存储,回到锁定态,用原口令解锁
        const salt = b64d(d.salt);
        localStorage.setItem(LC_KEY, JSON.stringify({ salt: d.salt, verify: d.verify, payload: d.payload }));
        st = d; LC.salt = salt; LC.verify = d.verify; LC.key = null; LC.notes = []; LC.sel = null;
...
/* ---------- 命令面板(Ctrl/Cmd+K) ---------- */
var palette: PaletteState = { open: false, items: [], sel: 0 };
function paletteIndex() {
  ...
  DSH.packageGroups.forEach(function (g) {
    g.pkgs.forEach(function (p) {
      ix.push({ type: '包', label: p, sub: 'packages/' + g.id + '/', act: function () { go('packages'); setTimeout(function () { var s = $<HTMLInputElement>('#pkgSearch'); if (s) { s.value = p; s.dispatchEvent(new Event('input')); } }, 60); } });
    });
  });
  DSH.tools.forEach(function (t) {
    ix.push({ type: '工具', label: t.n, sub: t.p, act: function () { go('tools'); setTimeout(function () { var s = $<HTMLInputElement>('#toolSearch'); if (s) { s.value = t.n.split(' ')[0]; s.dispatchEvent(new Event('input')); } }, 60); } });
  });
...
function openPalette() {
  ...
  var input = required<HTMLInputElement>('#paletteInput');
  input.focus();
...
function updateSel(items: HTMLElement[]): void {
...
function renderPalette(q: string): void {
  var box = required('#paletteResults');
  var needle = q.trim().toLowerCase();
  var hits: SearchItem[] = [];
...
function closeSidebar() {
  required('#sidebar').classList.remove('open');
  required('#sideMask').classList.remove('show');
  var mb = $('#menuBtn'); if (mb) mb.setAttribute('aria-expanded', 'false');
}
function bindChrome() {
  required('#menuBtn').addEventListener('click', function () {
    var open = !required('#sidebar').classList.contains('open');
    required('#sidebar').classList.toggle('open', open);
    required('#sideMask').classList.toggle('show', open);
    required('#menuBtn').setAttribute('aria-expanded', open ? 'true' : 'false');
  });
  ...
  required('#sideMask').addEventListener('click', closeSidebar);
  required('#themeBtn').addEventListener('click', function () {
    store.theme = store.theme === 'dark' ? 'light' : 'dark';
    save(); applyTheme();
  });
  required('#searchBtn').addEventListener('click', openPalette);
  document.addEventListener('keydown', function (e) {
    ...
    var tag = (e.target instanceof Element ? e.target.tagName : '').toLowerCase();
